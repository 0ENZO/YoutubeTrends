...
object Application {
  ...
  def getGlobalVideosDf(spark: SparkSession, lang_code_to_continent: Map[String, String]): DataFrame = {
    ...
    main_df.show(100 , truncate = false)
    main_df
  }
  ...
  def getMostWatchedCategoryForEachYear(spark: SparkSession, dfVideos: DataFrame): WindowSpec = {
    import spark.implicits._
...
//  def getMostTrendingsVideos(dfVideos: DataFrame): DataFrame = {
//
//    val df = dfVideos
